...
class PostCommentFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.imgBtnCommentSend.setOnClickListener {
            ...
            // 리스트 스크롤 시 키보드 내리기
            binding.rvComment.setOnScrollChangeListener { _, _, i2, _, i4 ->
                if (i2.minus(i4) >= 10) hideKeyboard(
                    binding.editComment,
                    requireContext()
                )
            }
        ...
        private fun sendComment() {
            ...
            if (!comment.content!!.isNullOrEmpty()) {
                RetrofitBuilder.commentApi.writeComment(accessToken, boardId, comment)
                    .enqueue(object :
                        Callback<Comment> {
                        override fun onResponse(call: Call<Comment>, response: Response<Comment>) {
                            if (response.isSuccessful) {
                                ...
                                Log.d(
                                    "API Success",
                                    "Written Comment Data is " + commentData.toString()
                                )
                            } else {
                                // 토큰 에러 처리
                                controlTokenError(
            ...
            RetrofitBuilder.commentApi.getCommentList(accessToken, boardId).enqueue(object :
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        adapter.setDeleteClickListener(object :
                            PostCommentListAdapter.OnDeleteClickListener {
                                ...
                                dialogBuilder.setMessage("삭제하시겠습니까?").setCancelable(false)
                                    .setPositiveButton(
                                        ...
                                        DialogInterface.OnClickListener { _, _ ->
                                            deleteComment(commentList?.get(position)?.commentId); fragmentRefresh()
                                        })
                                    .setNegativeButton(
                                        "취소",
